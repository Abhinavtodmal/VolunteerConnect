...
const TopVolunteers = () => {
  ...
  const loadShow = () => {
    if (!itemsRef.current[active]) return;

    const activeElement = itemsRef.current[active]!;
    ...
    for (let i = active + 1; i < itemsRef.current.length; i++) {
      ...
      stt++;
      element.style.transform = `translateX(${120 * stt}px) scale(${1 - 0.2 * stt}) perspective(16px) rotateY(-1deg)`;
    ...
    <div className="mb-20 dark:bg-black dark:text-white">
      ...
      >
        ...
        <motion.button
          ...
          onClick={() => setActive((prev) => (prev + 1 < items.length ? prev + 1 : prev))}
